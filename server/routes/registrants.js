...
router.get('/', requirePermission('registrants', 'view'), async (req, res) => {
  try {
    ...
    const registrants = await Registrant.find(query, 'fullName age sex shirtSize church ministry feeType paymentStatus amountPaid paymentMethod verifiedByTreasurer gcRef merchClaims merchClaimDates dateRegistered _id')
      .sort({ fullName: 1 })
      .skip(skip)
      .limit(limitNum);
